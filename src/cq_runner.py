...
DERIVED = URIRef(P + "wasDerivedFrom"); REPOSTARS = k("repoStars")
...
def build_index(gr):
    ...
    derived = {}                            # object/container -> source repo (v0.4.0)
    for s, _, o in gr.triples((None, DERIVED, None)):
        derived[s] = o
    repo_stars = {}                         # repo -> stars
    for s, _, o in gr.triples((None, REPOSTARS, None)):
        repo_stars[s] = int(o)
    return dict(typ=typ, inheres=inheres, bearer_aspects=bearer_aspects,
                ...
                derived=derived, repo_stars=repo_stars)
...
def run_cqs(gr):
    ...
    derived = ix["derived"]; repo_stars = ix["repo_stars"]
    ...
    def cq11():
        ...
        return len(out)
    t("CQ11", cq11)
    # CQ12 provenance: repos (with star counts) that host a privileged container
    def cq12():
        out = set()
        for a in typ.get("PrivilegedContainer", ()):
            repo = derived.get(inheres.get(a))
            if repo is not None and repo in repo_stars:
                out.add(repo)
        return len(out)
    t("CQ12", cq12)
    return results
